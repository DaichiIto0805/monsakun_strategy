...
for f in fnames:
    ...
    # df1_lv1 = df1_lv1.loc[:,['cluster0','check']].groupby(['cluster0'],as_index=False).mean()
    df1_lv2 = pd.merge(df1_lv2,dfcl2.loc[:,['InputID','cluster1']],on='InputID')
    # df1_lv2 = df1_lv2.loc[:,['cluster1','check']].groupby(['cluster1'],as_index=False).mean()
    df1_lv3 = pd.merge(df1_lv3,dfcl2.loc[:,['InputID','cluster2']],on='InputID')
    # df1_lv3 = df1_lv3.loc[:,['cluster2','check']].groupby(['cluster2'],as_index=False).mean()
